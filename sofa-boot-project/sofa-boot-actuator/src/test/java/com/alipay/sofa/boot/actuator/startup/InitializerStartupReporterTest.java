...
import com.alipay.sofa.boot.actuator.startup.spring.StartupSpringBootContextLoader;
...
public class InitializerStartupReporterTest {
    ...
    public void testStartupReporter() {
        ...
        Assert.assertTrue(baseStatList.stream().filter(stat -> stat.getName().equals("com.alipay.sofa.boot.actuator.startup.beans.StartupInitializer")).findFirst().get()
                .getCost() >= 10);
    }
}
